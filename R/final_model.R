...
finalModel <- function(sp,
  ...
  evall <- list.files(path = paste0(models.dir, "/", sp, "/present/partitions"), pattern = "evaluate",
    full.names = T)
  lista <- list()
  ...
  for (algo in algoritmos) {
    ...
    modelos.cont <- list.files(path = paste0(models.dir, "/", sp,"/present/partitions"), full.names = T,
      pattern = paste0(algo, "_cont_"))

    modelos.bin <- list.files(path = paste0(models.dir, "/", sp,"/present/partitions"), full.names = T,
	...
	raster::rasterOptions(setfileext = F)
        raster::writeRaster(x = final, filename = paste0(models.dir, "/", sp, "/present/",
