...
2. Model fitting and projecting: `do_any()` makes the ENM for one algorithm and partition; optionally, 
...
+ If a call to the function encounters previously written metadata, it will check if the current round has the same parameters and skip the data partitioning. If a previous metadata file is found but it has different metadata (i.e. the paramteres were modified), it will run the function.

__NOTE:__ `setup_sdmdata` will check if there's a prior folder structure and sdmdata.txt and metadata.txt files, in order to avoid repeating the data partitioning. If there is an inconsistency between the existing metadata and the current parameters, it will run the function with the current parameters. 
...
Here you can see the differences between the parameters of both functions. 
Actually `do_enm()` calls several instances of `do_any()` In practice you may only want to call `do_enm()`
but for parallelization by algorithm it may be better to call `do_any()` individually.

```{r args_do_any_do_enm}
args(do_any)
args(do_enm)
```
...
ensemble_files <-  list.files("~/modelR_test/1species/Eugenia florida DC./present/ensemble/",
                              recursive = T,
