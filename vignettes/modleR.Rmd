...
<img src="modleR.png" align="right" alt="" width="120" />
...
3. Partition joining: `final_model()` <!--selects or weighs (optionally) the partition models--> joins the partition models into a model per species per algorithm
...
`ensemble_model()` uses a `which_final` parameter -analog to `which_model` in `final_model()` to specify which final(s) model(s) (Figure 2) should be assembled together (the default is a mean of the raw continuous models: `which_final = c("raw_mean")`).
...
```{r check_ensemble, fig.width = 8, fig.height = 5, eval= TRUE}
