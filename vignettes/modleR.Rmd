...
sdm_list <- lapply(sdmdata_files, read.table, header=TRUE)
for (i in 1:length(coordenadas)) {
    sp <- species[i]
    occs <- coordenadas[[i]]
    sdmdata <- sdm_list[[i]]
...
Another option is to use the `purrr` package [@henry_purrr_2017]. Here we also use `dplyr` package for data manipulation.

```{r purrr example, eval = F, echo = F}
library(purrr)
library(dplyr)
sdmdata_list <- coordenadas %>% purrr::map(~ setup_sdmdata(species_name = names(.),
                                                           occurrences = .,
                                                           partition_type = "bootstrap",
                                                           boot_n = 5,
                                                           boot_proportion = 0.7,
